...
//INICIO CARGAR DATOS DE LOS SELECT
  ...
  if (true) { //Para alumnos
    ...
    for (var i = 0; i < listaAlumnosSeleccionados.length; i++) { //Recorre los alumnos seleccionados
      if (validarAlumnosEnBD(listaAlumnosSeleccionados[i]) != true) { //valida si existen ya en la BD antes de enviar el registro en la BD
        asignarAlumnoExhibicion(listaAlumnosSeleccionados[i], idExhibicion);
      }
    }
  }

  if (true) { //Para lugares
    limpiarLugaresNoSeleccionadosEnBD_YAsignarLugarExhibicion();
  }
  ...
  if (true) { //Para patrocinadores
    ...
    for (var i = 0; i < listaPatrocinadoresSeleccionados.length; i++) { //Recorre los alumnos seleccionados
      if (validarPatrocinadorEnBD(listaPatrocinadoresSeleccionados[i]) != true) { //valida si existen ya en la BD antes de enviar el registro en la BD
        asignarPatrocinadorExhibicion(listaPatrocinadoresSeleccionados[i], idExhibicion);
      }
    }
  }

  if (true) { //Para Organizaciones
    limpiarOrganizacionesNoSeleccionadosEnBD_YAsignarOrganizacionExhibicion();
  }
  location.href = 'listarExhibicion.html?id' + '=' + idExhibicion;
}

//-----------Inicio Para alumnos  BD--------------------
function limpiarAlumnosNoSeleccionadosEnBD() { // va a limpiar la BD de los alumnos no asigandos
  var aAlumnosEnBD = getIdAlumnosXExhibicion(idExhibicion);
  var bEncontrado = false;
  for (var i = 0; i < aAlumnosEnBD.length; i++) {
    if (validarAlumnoSeleccionado(aAlumnosEnBD[i].id_alumno) != true) {
      eliminarAlumnoEnBD(aAlumnosEnBD[i].id_alumno, idExhibicion);
    }
  }
}
...
function limpiarLugaresNoSeleccionadosEnBD_YAsignarLugarExhibicion() {
  var aLugarEnBD = getIdLugarXExhibicion(idExhibicion);
  if (aLugarEnBD.length != 0) {
    if (aLugarEnBD[0].id_lugar != listaLugaresSeleccionados[0]) {
      eliminarLugarEnBD(aLugarEnBD[0].id_lugar, idExhibicion);
      asignarLugarExhibicion(listaLugaresSeleccionados[0], idExhibicion);
    }
...
function limpiarOrganizacionesNoSeleccionadosEnBD_YAsignarOrganizacionExhibicion() {
  var aOrganizacionEnBD = getIdOrganizacionXExhibicion(idExhibicion);
  if (aOrganizacionEnBD.length != 0) {
    if (aOrganizacionEnBD[0].id_organizacion != listaOrganizacionesSeleccionadas[0]) {
      eliminarOrganizacionEnBD(aOrganizacionEnBD[0].id_organizacion, idExhibicion);
      asignarOrganizacionExhibicion(listaOrganizacionesSeleccionadas[0], idExhibicion);
    }
...
function limpiarPatrocinadoresNoSeleccionadosEnBD() { // va a limpiar la BD de los patrocinadores no asigandos
  var aPatrocinadoresEnBD = getIdPatrocinadoresXExhibicion(idExhibicion);
  var bEncontrado = false;
  for (var i = 0; i < aPatrocinadoresEnBD.length; i++) {
    if (validarPatrocinadorSeleccionado(aPatrocinadoresEnBD[i].id_patrocinador) != true) {
      eliminarPatrocinadorEnBD(aPatrocinadoresEnBD[i].id_patrocinador, idExhibicion);
    }
  }
}
